...
RULES_BASE_URL = "https://www.legis.iowa.gov/law/courtRules/courtRulesListings"
...
def build_html_report(report: ComplianceReport, version_stamp: str = "") -> str:
    ...
    subtitle_line = " — ".join(subtitle_parts) if subtitle_parts else "Iowa Rules of Appellate Procedure"
...
<div class="container">
  ...
  <footer>
    <p>Generated by JetBriefCheck — Iowa{f' {_esc(version_stamp)}' if version_stamp else ''}</p>
...
def _rule_link(rule_str: str) -> str:
    """Convert a rule citation like '6.903(2)(1)' into an HTML link.

    Links to the Iowa Legislature court rules page.
    """
    ...
    for part in parts:
        part = part.strip()
        url = RULES_BASE_URL
        links.append(f'<a href="{url}" target="_blank" class="rule-link">Rule {_esc(part)}</a>')
    return " / ".join(links)
